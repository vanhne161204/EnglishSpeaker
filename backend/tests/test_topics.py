...
async def test_create_and_list_topic(client: AsyncClient) -> None:
...
async def test_duplicate_slug_conflicts(client: AsyncClient) -> None:
    ...
    second = await client.post("/api/v1/topics", json=payload)
    assert second.status_code == 409
    assert second.json()["error"]["code"] == "conflict"


async def test_topic_get_update_delete(client: AsyncClient) -> None:
    created = await client.post("/api/v1/topics", json={"slug": "travel", "title": "Travel"})
    topic_id = created.json()["id"]

    got = await client.get(f"/api/v1/topics/{topic_id}")
    ...
    assert (await client.get(f"/api/v1/topics/{topic_id}")).status_code == 404


async def test_update_unknown_topic_returns_404(client: AsyncClient) -> None:
    resp = await client.patch(f"/api/v1/topics/{uuid.uuid4()}", json={"title": "x"})
